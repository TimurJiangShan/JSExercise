let getStudentFromId = id => studentRecords.find(record => record.id === id);
...
let paidStudentsToEnroll = () => 
	[
		...(
			studentRecords.filter(record => (record.paid === true && !currentEnrollment.includes(record.id)))
...
let remindUnpaid = recordIds => 
		printRecords(
			recordIds.filter(id => !getStudentFromId(id).paid)
		);
